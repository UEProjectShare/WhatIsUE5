...
void FWhiteNoiseCSManager::UpdateParameters(const FWhiteNoiseCSParameters& params)
{
	cachedParams = params;
...
void FWhiteNoiseCSManager::Execute_RenderThread(FRDGBuilder& GraphBuilder, const FSceneTextures& SceneTextures)
{
	...
	//GraphBuilder.RHICmdList.TransitionResource(EResourceTransitionAccess::ERWBarrier, EResourceTransitionPipeline::EGfxToCompute, ComputeShaderOutput->GetRenderTargetItem().UAV);
	GraphBuilder.RHICmdList.TransitionResource(ERHIAccess::UAVCompute, ComputeShaderOutput->GetRHI());
	...
	const TShaderMapRef<FWhiteNoiseCS> whiteNoiseCS(GetGlobalShaderMap(GMaxRHIFeatureLevel));

	//Dispatch the compute shader
	FComputeShaderUtils::Dispatch(GraphBuilder.RHICmdList, whiteNoiseCS, PassParameters,
	...
	GraphBuilder.RHICmdList.CopyTexture(ComputeShaderOutput->GetRenderTargetItem().ShaderResourceTexture, cachedParams.RenderTarget->GetRenderTargetResource()->TextureRHI, FRHICopyTextureInfo());
}
